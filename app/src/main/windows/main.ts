...
import { getProductVibeMode, setProductVibeMode } from "../lib/app-mode"
...
function registerIpcHandlers(): void {
  ...
  ipcMain.handle("app:get-product-vibe-mode", () => getProductVibeMode())
  ipcMain.handle("app:set-product-vibe-mode", (_event, enabled: boolean) => {
    return setProductVibeMode(enabled === true)
  })
  ...
  const validateSender = (event: Electron.IpcMainInvokeEvent): boolean => {
    ...
    try {
      ...
      const trusted = ["vilda.dev", "localhost", "127.0.0.1"]
...
export function createWindow(options?: { chatId?: string; subChatId?: string }): BrowserWindow {
  ...
  console.log("[Main] ProductVibe mode:", productVibeMode)
