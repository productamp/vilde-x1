...
function AppContent() {
  ...
  useEffect(() => {
    ...
    setDesktopView(null)
  }, [productVibeMode, setDesktopView, setSelectedTeamId])
...
export function App() {
  const [productVibeReady, setProductVibeReady] = useState(false)

  useEffect(() => {
    let cancelled = false

    async function syncProductVibeMode() {
      const desktopApi = window.desktopApi
      if (!desktopApi?.getProductVibeMode) {
        if (!cancelled) setProductVibeReady(true)
        return
      }

      try {
        const persistedMode = await desktopApi.getProductVibeMode()
        if (cancelled) return
        appStore.set(productVibeModeAtom, persistedMode)
      } catch (error) {
        console.warn("[App] Failed to load ProductVibe mode:", error)
      } finally {
        if (!cancelled) setProductVibeReady(true)
      }
    }
