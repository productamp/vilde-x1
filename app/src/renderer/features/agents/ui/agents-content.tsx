...
export function AgentsContent() {
  ...
  const { data: teams } = api.teams.getUserTeams.useQuery(undefined, {
    enabled: !!selectedTeamId && !productVibeMode,
  })
  ...
  const { data: automationsData } = useQuery({
    ...
    enabled: !!selectedTeamId && !betaAutomationsEnabled && !productVibeMode,
    staleTime: Infinity,
  ...
  const { data: localPreview } = trpc.preview.ensureForChat.useQuery(
    { chatId: selectedChatId! },
    {
      enabled: !!selectedChatId && !!worktreePath,
      staleTime: Infinity,
      refetchOnWindowFocus: false,
      retry: 1,
    },
  ...
  const previewBaseUrl =
    ...
    (hasRemotePreview
      ? `https://${chatData!.sandbox_id!}-${chatMeta?.sandboxConfig?.port!}.csb.app`
      : null)
  const canShowPreview = !!previewBaseUrl
