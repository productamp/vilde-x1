...
export function ChatView({
  ...
  const [, setDetailsSidebarTab] = useAtom(detailsSidebarTabAtom)
  ...
  const autoOpenedPreviewChatRef = useRef<string | null>(null)
  ...
  const { data: localPreview } = trpc.preview.ensureForChat.useQuery(
    { chatId },
    {
  ...
  useEffect(() => {
    if ((!canOpenPreview || isUnifiedSidebarEnabled) && isPreviewSidebarOpen) {
      setIsPreviewSidebarOpen(false)
    }
  }, [canOpenPreview, isUnifiedSidebarEnabled, isPreviewSidebarOpen, setIsPreviewSidebarOpen])

  useEffect(() => {
    if (!productVibeMode || isMobileFullscreen) return
    if (isUnifiedSidebarEnabled) {
      if (canOpenPreview && autoOpenedPreviewChatRef.current !== chatId) {
        autoOpenedPreviewChatRef.current = chatId
        setIsDetailsSidebarOpen(true)
        setDetailsSidebarTab("preview")
      }
      return
    }
  ...
  const previewReloadTimerRef = useRef<NodeJS.Timeout | null>(null)
  ...
  const schedulePreviewReload = useCallback(() => {
    ...
    previewReloadTimerRef.current = setTimeout(() => {
      ...
      window.dispatchEvent(
        new CustomEvent("agent-preview-reload", {
          detail: { chatId },
        }),
      )
    }, 150)
  ...
  }, [scheduleDiffRefresh, schedulePreviewReload])

  // Listen for file changes from Claude Write/Edit tools and refresh diff
  useFileChangeListener(worktreePath, { onChange: handleWorkspaceChange })

  // Subscribe to GitWatcher for real-time file system monitoring (chokidar on main process)
  useGitWatcher(worktreePath, { onChange: handleWorkspaceChange, debounceMs: 200 })
