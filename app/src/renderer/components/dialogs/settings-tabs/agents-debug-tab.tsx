...
export function AgentsDebugTab() {
  ...
  const [productVibeMode, setProductVibeMode] = useAtom(productVibeModeAtom)
  ...
  const handleProductVibeToggle = async (enabled: boolean) => {
    try {
      await window.desktopApi?.setProductVibeMode?.(enabled)
      setProductVibeMode(enabled)
      toast.success(
        enabled ? "ProductVibe mode enabled" : "ProductVibe mode disabled",
        {
          description: "Reloading to apply auth and onboarding changes.",
        },
      )
      setTimeout(() => window.location.reload(), 150)
    } catch (error) {
      toast.error("Failed to update ProductVibe mode")
      console.error(error)
    }
  }
        ...
        <div className="rounded-lg border bg-muted/30 divide-y">
          <div className="flex items-center justify-between p-3 gap-4">
            <div className="space-y-1">
              <div className="text-sm font-medium">ProductVibe Mode</div>
              <p className="text-sm text-muted-foreground">
                Bypass desktop auth and load the simplified website-builder flow.
              </p>
            </div>
            <Switch
              checked={productVibeMode}
              onCheckedChange={handleProductVibeToggle}
            />
          </div>
        </div>
      </div>
