...
export function ClaudeLoginModal({
  ...
  const authenticateLocalCliMutation =
    trpc.claudeCode.authenticateLocalCli.useMutation()
  ...
  const handleConnectClick = useCallback(async () => {
    if (authMode === "local-cli") {
      try {
        await authenticateLocalCliMutation.mutateAsync()
        setProductVibeMode(true)
        handleAuthSuccess()
      } catch (err) {
        setFlowState({
          step: "error",
          message:
            err instanceof Error ? err.message : "Failed to connect Claude Code",
        })
      }
      return
    }
          ...
          <div className="text-center space-y-4">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">
                {authMode === "local-cli"
                  ? "Connect your local Claude Code installation"
          ...
          <div className="space-y-6">
            ...
            {!urlOpened && flowState.step !== "has_url" && flowState.step !== "error" && (
              <Button
                ...
                disabled={
                  authMode === "local-cli"
                    ? authenticateLocalCliMutation.isPending
                    : userClickedConnect && isLoadingAuth
                }
              >
                {(authMode === "local-cli" && authenticateLocalCliMutation.isPending) ||
                (userClickedConnect && isLoadingAuth) ? (
                  <IconSpinner className="h-4 w-4" />
                ) : (
                  authMode === "local-cli" ? "Connect Claude Code" : "Connect"
                )}
              </Button>
            )}

            {/* Code Input - Show after URL is opened or if has_url */}
            {authMode !== "local-cli" && (urlOpened ||
              flowState.step === "has_url" ||
